...
export default function ViewStation() {
    ...
    useEffect(() => {
        const fetchStation = async () => {
            try {
                ...
                console.log("view station ", res);

                const stationsRes = await axiosInstance.get(
                    endpoint.station.showall
                );
                console.log("view all station ", stationsRes);

                if (res.data && stationsRes.data) {
                    setStation(res.data.data || {});
                    setAllStations(stationsRes.data.data.users || []);

                    // ✅ Extract policeList directly from response
                    const currentStation = stationsRes.data.data.users.find(
                        (s) => s.id === id
                    );
    ...
    const { stationName, contactNumber, location, isActive } = station;
